...
def main():
    ...
    train_method = default_config['TrainMethod'] #icm

    #Uncomment these two lines for atari breakoutnoframeskip-v4
    env_id = default_config['EnvID'] #BreakoutNoFrameskip-v4
    ...
    if env_type == 'mario':
        ...
        env = gym.make(env_id)
    elif env_type == 'minerl':
        env = gym.make(env_id) # A MineRLNavigate-v0 env
    else:
    ...
    obs_rms = RunningMeanStd(shape=(1, 8, 64, 64))
    ...
    elif default_config['EnvType'] == 'minerl':
        env_type = MineRlEnvironment
    else:
        raise NotImplementedError
    ...
    states = np.zeros([num_worker, 8, 64, 64])
    ...
    while True:
        ...
        total_state = np.stack(total_state).transpose([1, 0, 2, 3, 4]).reshape([-1, 8, 64, 64])
        total_next_state = np.stack(total_next_state).transpose([1, 0, 2, 3, 4]).reshape([-1, 8, 64, 64])
